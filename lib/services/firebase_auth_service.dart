...
class AuthService{
  ...
  Future registerWithEmailAndPassword(String email, String password, String fname, String lname,String username, String phone, String address) async{
    ...
    try{
      ...
      await FirestoreService(uid: user.uid).setOrUpdateUserData(fname, lname, email, username, phone, address,indexList); //create an instance of the firestore db service (need to pass UID!) and then set the data for the first time
      return _userFromFirebaseUser(user); // transform to our custom model
    } catch(e){
      print(e.toString());
      return null;
