...
class AuthService {
  final FirebaseAuth _auth = FirebaseAuth
      .instance; //instance so we can use firebase auth. We use this whenever we want to interact with any authentication functionality inside this class (sign in / out / register, etc)
  ...
  Future registerWithEmailAndPassword(
    ...
    try {
      ...
      await FirestoreService(uid: user.uid).setOrUpdateUserData(
          fname,
          lname,
          email,
          username,
          phone,
          address,
          indexList, pin); //create an instance of the firestore db service (need to pass UID!) and then set the data for the first time
      return _userFromFirebaseUser(user); // transform to our custom model
    } catch (e) {
      print(e.toString());
      return null;
