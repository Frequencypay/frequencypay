...
class ContractService {
  ...
  Future<RepaymentOverview> getRepaymentOverview() async {
    ...
    if (activeContracts.length == 0) {
      ...
      for (int index = 0; index < activeContracts.length; index++) {
        ...
        // ignore: todo
        currentAmountPaid =
            0; //TODO Make this actually read the amount paid back
  ...
  void acceptContractRequest(Contract contract) {
    ...
    List<ScheduledTransaction> repaymentTransactions =
        _computeFutureTransactions(contract, DateTime.now());
  ...
  List<ScheduledTransaction> _computeFutureTransactions(
      Contract contract, DateTime startTime) {
    ...
    Duration timeBetweenTransactions =
        Duration(days: contract.terms.frequencyWeeks * 7);
    ...
    if (amountRepaid < contract.terms.amount) {
      ...
      currentTransaction = ScheduledTransaction(
          contract.terms.amount - amountRepaid, currentTime);
      transactions.add(currentTransaction);
    }
    ...
    for (int index = 0; index < activeContracts.length; index++) {
      ...
      for (int index2 = 0; index2 < transactions.length; index2++) {
        ...
        if (currentTransaction.time.isAfter(DateTime.now())) {
          ...
          payments.add(DisplayTransaction(
              current, currentTransaction.amount, currentTransaction.time));
        }
      }
    }
