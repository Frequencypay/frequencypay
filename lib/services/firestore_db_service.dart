...
class FirestoreService {
  ...
  FirestoreService({this.uid, this.value}) {
  ...
  final CollectionReference contractCollection =
      Firestore.instance.collection('contracts');
  final CollectionReference userBillsCollection =
      Firestore.instance.collection('user_bills'); //NOT USED YET
  ...
  Future setOrUpdateUserData(String fname, String lname, String email,
      String username, String phone, String address, List indexList) async {
  ...
  Future createContractRequest(
      String requester,
      String loaner,
      String requesterName,
      String loanerName,
      String dueDate,
      RepaymentTerms terms) async {
  ...
  void acceptEstablishContract(
      Contract contract, List repaymentTransactions) async {
  ...
  void _establishContract(
      String uid, Contract contract, List transactions) async {
...
// THIS IS THE FUNCTION THAT TRANSFORMS THE CONTRACT DATA WE GET FROM DB INTO OUR CUSTOM contract model
  ...
  Stream<List<Contract>> retrieveContracts(ContractSearchQuery query) async* {
    ...
    if (query.requirementMode == REQUIREMENT_MODE.REQUESTER) {
      contractsStream = contractCollection
          .where('state', isEqualTo: query.state.toString())
          .where('requester', isEqualTo: currentUser.uid)
          .snapshots()
          .map(_activeContractListFromSnapshot);
    }
  ...
  Stream<QuerySnapshot> get userSearchData {
    return (value == null)
        ...
        : Firestore.instance
            ...
            .where("searchIndex", arrayContains: value)
            .snapshots();
  }
